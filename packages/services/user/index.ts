...
import { createUserWithEmailAndPasswordInput, CreateUserWithEmailAndPasswordInputType, generateUserTokenPayloadInput, GenerateUserTokenPayloadInputType, loginUserWithEmailAndPasswordInput, LoginUserWithEmailAndPasswordInputType } from "./model";
import { env } from "../env";
...
class UserService {
  ...
  public async getUserById(id: string) {
    ...
    }).from(usersTable).where(eq(usersTable.id, id)).execute();
    if(!result || result.length === 0) {
